...
class Process_Package:
    def __init__(self,global_queue : queue.Queue,architecture,N):
        ...
        self.buffor = m.Queue()
        self.p = m.Process(target=slave_process, args=(self.conn1,self.buffor,architecture,N),kwargs={"nr_of_samplings_in_row":50,"process_id":1})
        pass
...
def main_process(process_number = 3, instance_count = 100):
    ...
    proc_lst = []
    # p = m.Process(target=slave_process, args=(conn1,buffor,asc.architecture,asc.N),kwargs={"nr_of_samplings_in_row":100,"process_id":1})
